...
const { BUS_HOST, BUS_PORT, SERVICE_CODE } = require('../config/busConfig');
...
async function registerEmployee({ nombre, apellido, email, password, telefono, rol }) {
  return new Promise((resolve, reject) => {
    ...
    clientSocket.connect(BUS_PORT, BUS_HOST, () => {
      const request = buildTransaction(SERVICE_CODE, `registrar;${nombre};${apellido};${email};${password};${telefono};${rol}`);
    ...
    clientSocket.on('data', (data) => {
      ...
      messages.forEach((message) => {
        try {
          ...
          if (parsed.serviceName === SERVICE_CODE && operation === 'registrar') {
            if (parsed.status === 'OK') {
